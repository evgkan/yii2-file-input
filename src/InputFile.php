...
class InputFile extends \mihaildev\elfinder\InputFile {
    ...
    public function run() {
        ...
        if (!empty($this->callbackFunction)) {
            $this->getView()->registerJs("ElFinderFileCallback.register(" . Json::encode($this->options['id']) . "," . Json::encode($this->callbackFunction) . ");$('#" . $this->buttonOptions['id'] . "').click(function(){ElFinderFileCallback.openManager(" . Json::encode($this->_managerOptions) . ");});");
        } else {
